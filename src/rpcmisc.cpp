...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "validateaddress \"csbankaddress\"\n"
            "\nReturn information about the given csbank address.\n"
            "\nArguments:\n"
            "1. \"csbankaddress\"     (string, required) The csbank address to validate\n"
            "\nResult:\n"
            "{\n"
            "  \"isvalid\" : true|false,         (boolean) If the address is valid or not. If not, this is the only property returned.\n"
            "  \"address\" : \"csbankaddress\",   (string) The csbank address validated\n"
...
#ifdef ENABLE_WALLET
        // Case 1: CSbank address and we have full public key:
...
Value createmultisig(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 2)
    {
        string msg = "createmultisig nrequired [\"key\",...]\n"
            ...
            "2. \"keys\"       (string, required) A json array of keys which are csbank addresses or hex-encoded public keys\n"
            "     [\n"
            "       \"key\"    (string) csbank address or hex-encoded public key\n"
            "       ,...\n"
            "     ]\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage \"csbankaddress\" \"signature\" \"message\"\n"
            "\nVerify a signed message\n"
            "\nArguments:\n"
            "1. \"csbankaddress\"    (string, required) The csbank address to use for the signature.\n"
