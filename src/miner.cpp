...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
	...
	// Found a solution
	{
		...
		return error("CSbankMiner : generated block is stale");
	}
	...
	return error("CSbankMiner : ProcessNewBlock, block not accepted");
...
{
	LogPrintf("CSbankMiner started\n");
	SetThreadPriority(THREAD_PRIORITY_LOWEST);
	RenameThread("csbank-miner");
	MilliSleep(1000);
...
//		LogPrintf("BitcoinMiner:: pblocktemplate.get() == 0 cointinue.....................................\n");
					...
					if (!pwallet->IsLocked()) {
						if (!pwallet->TopUpKeyPool()) {
							LogPrintf("Error in CSbankMiner: Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
							return;
						}
					}
					continue;
				...
				LogPrintf("Running CSbankMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
		...
		{
			LogPrintf("CSbankMiner terminated\n");
			throw;
		}
		...
		{
			LogPrintf("CSbankMiner runtime error: %s\n", e.what());
			return;
		}
