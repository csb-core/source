...
//    LogPrintf("IsValidNetAddr()---- IsReachable(addr):%d, addr.IsRoutable:%d, address:%s\n", IsReachable(addr), addr.IsRoutable(), addr.ToString().c_str());
...
bool CMasternodeBroadcast::CheckInputsAndAdd(int& nDoS)
{
    ...
    if (mi != mapBlockIndex.end() && (*mi).second) {
        CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 1000 CSB tx -> 1 confirmation
