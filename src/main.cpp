...
#if defined(NDEBUG)
# error "CSbank cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "CSbank Signed Message:\n";
...
void RegisterValidationInterface(CValidationInterface* pwalletIn) {
	g_signals.SyncTransaction.connect(
			boost::bind(&CValidationInterface::SyncTransaction, pwalletIn, _1,
					_2));
	// g_signals.EraseTransaction.connect(boost::bind(&CValidationInterface::EraseFromWallet, pwalletIn, _1));
	g_signals.UpdatedTransaction.connect(
			boost::bind(&CValidationInterface::UpdatedTransaction, pwalletIn,
					_1));
	...
	g_signals.Broadcast.connect(
			boost::bind(&CValidationInterface::ResendWalletTransactions,
					pwalletIn));
	g_signals.BlockChecked.connect(
			boost::bind(&CValidationInterface::BlockChecked, pwalletIn, _1,
					_2));
}
...
void InitializeNode(NodeId nodeid, const CNode *pnode) {
	LOCK(cs_main);
	CNodeState &state = mapNodeState.insert(
			std::make_pair(nodeid, CNodeState())).first->second;
...
void MarkBlockAsInFlight(NodeId nodeid, const uint256& hash,
	...
	QueuedBlock newentry = { hash, pindex, GetTimeMicros(),
			nQueuedValidatedHeaders, pindex != NULL };
	nQueuedValidatedHeaders += newentry.fValidatedHeaders;
	list<QueuedBlock>::iterator it = state->vBlocksInFlight.insert(
			state->vBlocksInFlight.end(), newentry);
	state->nBlocksInFlight++;
...
void ProcessBlockAvailability(NodeId nodeid) {
	...
	if (state->hashLastUnknownBlock != 0) {
		BlockMap::iterator itOld = mapBlockIndex.find(
				state->hashLastUnknownBlock);
...
void FindNextBlocksToDownload(NodeId nodeid, unsigned int count,
		std::vector<CBlockIndex*>& vBlocks, NodeId& nodeStaller) {
	if (count == 0)
		return;
	...
	int nWindowEnd = state->pindexLastCommonBlock->nHeight
			+ BLOCK_DOWNLOAD_WINDOW;
	int nMaxHeight = std::min<int>(state->pindexBestKnownBlock->nHeight,
			nWindowEnd + 1);
	NodeId waitingfor = -1;
...
void static EraseOrphanTx(uint256 hash) {
	...
	BOOST_FOREACH(const CTxIn& txin, it->second.tx.vin) {
		map<uint256, set<uint256> >::iterator itPrev =
				mapOrphanTransactionsByPrev.find(txin.prevout.hash);
...
bool AreInputsStandard(const CTransaction& tx,
	...
	for (unsigned int i = 0; i < tx.vin.size(); i++) {
		...
		if (!EvalScript(stack, tx.vin[i].scriptSig, false,
		...
		if (whichType == TX_SCRIPTHASH) {
			...
			if (Solver(subscript, whichType2, vSolutions2)) {
				int tmpExpected = ScriptSigArgsExpected(whichType2,
						vSolutions2);
				if (tmpExpected < 0)
					return false;
...
int GetInputAgeIX(uint256 nTXHash, CTxIn& vin) {
	...
	if (nResult < 6) {
		std::map<uint256, CTransactionLock>::iterator i = mapTxLocks.find(
				nTXHash);
		if (i != mapTxLocks.end()) {
...
bool GetCoinAge(const CTransaction& tx, const unsigned int nTxTime,
		uint64_t& nCoinAge) {
	...
	BOOST_FOREACH (const CTxIn& txin, tx.vin) {
		...
		if (nTxTime < prevblock.nTime) {
			LogPrintf(
					"GetCoinAge: Timestamp Violation: txtime less than txPrev.nTime");
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state) {
	...
	BOOST_FOREACH(const CTxOut& txout, tx.vout) {
		...
		if (!MoneyRange(nValueOut))
			return state.DoS(100,
					error("CheckTransaction() : txout total out of range"),
	...
	if (tx.IsCoinBase()) {
		if (tx.vin[0].scriptSig.size() < 2 || tx.vin[0].scriptSig.size() > 100)
			return state.DoS(100,
					error("CheckTransaction() : coinbase script size"),
					REJECT_INVALID, "bad-cb-length");
	} else {
		BOOST_FOREACH(const CTxIn& txin, tx.vin)
			if (txin.prevout.IsNull())
				return state.DoS(10,
						error("CheckTransaction() : prevout is null"),
...
bool AcceptToMemoryPool(CTxMemPool& pool, CValidationState &state,
		const CTransaction &tx, bool fLimitFree, bool* pfMissingInputs,
	...
	if (tx.IsCoinStake())
		return state.DoS(100,
				error("AcceptToMemoryPool: coinstake as individual tx"),
	...
	if (Params().RequireStandard() && !IsStandardTx(tx, reason))
		return state.DoS(0,
				error("AcceptToMemoryPool : nonstandard transaction: %s",
						reason), REJECT_NONSTANDARD, reason);
...

	{
		...
		CTxMemPoolEntry entry(tx, nFees, GetTime(), dPriority,
				chainActive.Height());
		unsigned int nSize = entry.GetTxSize();

		// Don't accept it if it can't get into a block
		if (mapObfuscationBroadcastTxes.count(hash)) {
			mempool.PrioritiseTransaction(hash, hash.ToString(), 1000,
					0.1 * COIN);
		} else if (!ignoreFees) {
			...
			if (GetBoolArg("-relaypriority", true)
					...
					&& !AllowFree(
							view.GetPriority(tx, chainActive.Height() + 1))) {
...
bool AcceptableInputs(CTxMemPool& pool, CValidationState& state,
		const CTransaction& tx, bool fLimitFree, bool* pfMissingInputs,
		bool fRejectInsaneFee, bool isDSTX) {
	AssertLockHeld(cs_main);
	if (pfMissingInputs)
		*pfMissingInputs = false;
	...
	if (tx.IsCoinBase())
		return state.DoS(100,
				error("AcceptableInputs: : coinbase as individual tx"),
				REJECT_INVALID, "coinbase");
			...
			if (mapLockedInputs[in.prevout] != tx.GetHash()) {
				return state.DoS(0,
						error(
								...
								reason), REJECT_INVALID, "tx-lock-conflict");
			}
		}
	}
...

	{
		...
		CAmount nValueIn = 0;
		{
			...
			if (!view.HaveInputs(tx))
				return state.Invalid(
						error("AcceptableInputs : inputs already spent"),
						REJECT_DUPLICATE, "bad-txns-inputs-spent");
		...
		if (isDSTX) {
			...
			if (fLimitFree && nFees < txMinFee)
				return state.DoS(0,
						error("AcceptableInputs : not enough fees %s, %d < %d",
								hash.ToString(), nFees, txMinFee),
						REJECT_INSUFFICIENTFEE, "insufficient fee");
		...
		if (!CheckInputs(tx, state, view, false, STANDARD_SCRIPT_VERIFY_FLAGS,
				true)) {
			return error("AcceptableInputs: : ConnectInputs failed %s",
					hash.ToString());
		}
...
//    LogPrintf("GetProofOfStakeReward:: create=%s nCoinAge=%d\n", FormatMoney(nSubsidy), nCoinAge);
	if (nHeight > 800 && nHeight <= 2000) {
		nSubsidy = 10 * COIN;		//30%  of reward
...
void static InvalidBlockFound(CBlockIndex *pindex,
	...
	if (state.IsInvalid(nDoS)) {
		std::map<uint256, NodeId>::iterator it = mapBlockSource.find(
				pindex->GetBlockHash());
		if (it != mapBlockSource.end() && State(it->second)) {
			CBlockReject reject = { state.GetRejectCode(),
					state.GetRejectReason().substr(0,
							MAX_REJECT_MESSAGE_LENGTH), pindex->GetBlockHash() };
...
void UpdateCoins(const CTransaction& tx, CValidationState &state,
		CCoinsViewCache &inputs, CTxUndo &txundo, int nHeight) {
	// mark inputs spent
	if (!tx.IsCoinBase()) {
		txundo.vprevout.reserve(tx.vin.size());
		BOOST_FOREACH(const CTxIn &txin, tx.vin) {
			txundo.vprevout.push_back(CTxInUndo());
			bool ret = inputs.ModifyCoins(txin.prevout.hash)->Spend(
					txin.prevout, txundo.vprevout.back());
			assert(ret);
		}
	}
...
bool CScriptCheck::operator()() {
	...
	if (!VerifyScript(scriptSig, scriptPubKey, nFlags,
			CachingTransactionSignatureChecker(ptxTo, nIn, cacheStore),
			&error)) {
...
bool CheckInputs(const CTransaction& tx, CValidationState &state,
		const CCoinsViewCache &inputs, bool fScriptChecks, unsigned int flags,
		bool cacheStore, std::vector<CScriptCheck> *pvChecks) {
	if (!tx.IsCoinBase()) {
		if (pvChecks)
		...
		CBlockIndex *pindexPrev =
				mapBlockIndex.find(inputs.GetBestBlock())->second;
				...
				if (nSpendHeight - coins->nHeight < COINBASE_MATURITY)
					return state.Invalid(
							error(
									...
									coins->IsCoinStake()), REJECT_INVALID,
			...
			if (nValueIn < tx.GetValueOut())
				return state.DoS(100,
						error(
								...
								FormatMoney(tx.GetValueOut())), REJECT_INVALID,
			...
			if (nTxFee < 0)
				return state.DoS(100,
						error("CheckInputs() : %s nTxFee < 0",
								tx.GetHash().ToString()), REJECT_INVALID,
						"bad-txns-fee-negative");
			nFees += nTxFee;
			if (!MoneyRange(nFees))
				return state.DoS(100,
						error("CheckInputs() : nFees out of range"),
...
bool DisconnectBlock(CBlock& block, CValidationState& state,
		CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean) {
...
bool ConnectBlock(const CBlock& block, CValidationState& state,
	...
	uint256 hashPrevBlock =
			pindex->pprev == NULL ? uint256(0) : pindex->pprev->GetBlockHash();
	...
	bool fScriptChecks = pindex->nHeight
...
//		LogPrintf("ConnectInputs() : tx.GetValueOut(): %d, nValueIn: %dxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx\n", tx.GetValueOut(), nValueIn);
				...
				if (pindex->nHeight >= Params().FirstMasternodePaymentBlock()) {
					CAmount blockValue = GetProofOfStakeReward(pindex->nHeight,
							nCoinAge);
					nStakeReward -= GetMasternodePayment(pindex->nHeight,
							blockValue);
				}

				CAmount nActualReward = GetProofOfStakeReward(pindex->nHeight,
						nCoinAge);
...
//                    return state.DoS(100, error("ConnectInputs() : %s stake reward exceeded", tx.GetHash().ToString().substr(0,10).c_str()));
		...
		UpdateCoins(tx, state, view,
				i == 0 ? undoDummy : blockundo.vtxundo.back(), pindex->nHeight);
	...
	if (!IsInitialBlockDownload()) {
		...
		if (block.IsProofOfWork()) {
			ExpectedPay = GetBlockValue(pindex->nHeight) + nFees
					+ GetMasternodePayment(pindex->nHeight,
							GetBlockValue(pindex->nHeight));
...
enum FlushStateMode {
	FLUSH_STATE_IF_NEEDED, FLUSH_STATE_PERIODIC, FLUSH_STATE_ALWAYS
};
	...
	try {
		if ((mode == FLUSH_STATE_ALWAYS)
				|| ((mode == FLUSH_STATE_PERIODIC
						|| mode == FLUSH_STATE_IF_NEEDED)
						&& pcoinsTip->GetCacheSize() > nCoinCacheSize)
				|| (mode == FLUSH_STATE_PERIODIC
						&& GetTimeMicros()
								> nLastWrite + DATABASE_WRITE_INTERVAL * 1000000)) {
			...
			for (set<int>::iterator it = setDirtyFileInfo.begin();
			...
			for (set<CBlockIndex*>::iterator it = setDirtyBlockIndex.begin();
		...
		return state.Abort(
				std::string("System error while flushing: ") + e.what());
	}
	return true;
}
...
bool DisconnectBlockAndInputs(CValidationState& state, CTransaction txLock) {
	...
	for (unsigned int i = 1;
			BlockReading && BlockReading->nHeight > 0 && !foundConflictingTx
					&& i < 6; i++) {
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue,
	...
	ret_val = blockValue * 90 / 100; 	//90% of the reward
	return ret_val;
}
...
static bool ActivateBestChainStep(CValidationState &state,
		CBlockIndex *pindexMostWork, CBlock *pblock) {
	AssertLockHeld(cs_main);
	...
	while (fContinue && nHeight != pindexMostWork->nHeight) {
		...
		BOOST_REVERSE_FOREACH(CBlockIndex *pindexConnect, vpindexToConnect) {
			if (!ConnectTip(state, pindexConnect,
					pindexConnect == pindexMostWork ? pblock : NULL)) {
				if (state.IsInvalid()) {
...
CBlockIndex* AddToBlockIndex(const CBlockHeader& block) {
	...
	if (miPrev != mapBlockIndex.end()) {
		...
		pindexNew->bnChainTrust = (
				pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0)
		...
		if (!CheckStakeModifierCheckpoints(pindexNew->nHeight,
			...
			LogPrintf(
					...
					pindexNew->nHeight,
					boost::lexical_cast<std::string>(nStakeModifier));
	}
	pindexNew->nChainWork =
...
bool FindBlockPos(CValidationState &state, CDiskBlockPos &pos,
	...
	if (fKnown)
		vinfoBlockFile[nFile].nSize = std::max(pos.nPos + nAddSize,
...
//    LogPrintf("CheckBlock::----------------BoostTransaction --------------passedpppppppppppppppppppppppppppppppp\n");
		...
		BOOST_FOREACH (const CTransaction& tx, block.vtx) {
			if (!tx.IsCoinBase()) {
				...
				BOOST_FOREACH (const CTxIn& in, tx.vin) {
					if (mapLockedInputs.count(in.prevout)) {
						if (mapLockedInputs[in.prevout] != tx.GetHash()) {
							mapRejectedBlocks.insert(
									make_pair(block.GetHash(), GetTime()));
							LogPrintf(
	...
	LogPrintf(
			"CheckBlock::----------------masternode start --------------passedpppppppppppppppppppppppppppppppp\n");
...
//	LogPrintf("CheckBlock::----------------IsInitialBlockDownload if Start --------------passedpppppppppppppppppppppppppppppppp\n");
		if (nHeight != 0 && !IsInitialBlockDownload()) {
			LogPrintf(
					"CheckBlock::----------------IsBlockPayeeValid start --------------passedpppppppppppppppppppppppppppppppp\n");
			if (!IsBlockPayeeValid(block, nHeight)) {
				LogPrintf(
						"CheckBlock::----------------IsBlockPayeeValid false --------------passedpppppppppppppppppppppppppppppppp\n");
	...
	LogPrintf(
			"CheckBlock::----------------SUCCESSFULL --------------passedpppppppppppppppppppppppppppppppp\n");
	// -------------------------------------------
...
bool ContextualCheckBlockHeader(const CBlockHeader& block,
		CValidationState& state, CBlockIndex * const pindexPrev) {
	uint256 hash = block.GetHash();
	...
	if (block.GetBlockTime() <= pindexPrev->GetMedianTimePast())
		return state.Invalid(
				error("%s : block's timestamp is too early", __func__),
	...
	if (!Checkpoints::CheckBlock(nHeight, hash))
		return state.DoS(100,
				error("%s : rejected by checkpoint lock-in at %d", __func__,
						nHeight), REJECT_CHECKPOINT, "checkpoint mismatch");
	...
	if (block.nVersion <= 112
			&& CBlockIndex::IsSuperMajority(113, pindexPrev,
					...
					Params().ToCheckBlockUpgradeMajority())) {
		return state.Invalid(
				error("%s : rejected nVersion(%d)<=112 block", __func__,
						block.nVersion), REJECT_OBSOLETE, "bad-version");
	}
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state,
	...
	BOOST_FOREACH(const CTransaction& tx, block.vtx)
		if (!IsFinalTx(tx, nHeight, block.GetBlockTime())) {
			return state.DoS(10,
					error("%s : contains a non-final transaction", __func__),
	...
	if (block.nVersion >= 2) {
		...
		if (CBlockIndex::IsSuperMajority(2, pindexPrev, 750, 1000)) {
			...
			if (block.vtx[0].vin[0].scriptSig.size() < expect.size()
				...
				return state.DoS(100,
						error("%s : block height mismatch in coinbase",
								__func__), REJECT_INVALID, "bad-cb-height");
		}
	}
...
bool AcceptBlockHeader(const CBlockHeader& block, CValidationState& state,
		CBlockIndex** ppindex) {
	AssertLockHeld(cs_main);
	// Check for duplicate
	uint256 hash = block.GetHash();
	...
	if (miSelf != mapBlockIndex.end()) {
		...
		if (pindex->nStatus & BLOCK_FAILED_MASK)
			return state.Invalid(
					error("%s : block is marked invalid", __func__), 0,
					"duplicate");
		return true;
	}
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex,
		CDiskBlockPos* dbp) {
	AssertLockHeld(cs_main);
...
void RestartPro() {
	...
	string strStop;
	string strStart;
	strStop = strPath + "/csbank-cli stop";
	strStart = strPath + "/csbankd";
...
bool TestBlockValidity(CValidationState &state, const CBlock& block,
		CBlockIndex * const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot) {
...
//	LogPrintf("TestBlockValidity::----ContextualCheckBlockHeader---FALSE--------0000000000000000000000000000000000000000000000000\n");
	...
	LogPrintf(
			"TestBlockValidity::----ContextualCheckBlockHeader--------passed0000000000000000000000000000000000000000000000000\n");

	if (!CheckBlock(block, state, fCheckPOW, fCheckMerkleRoot)) {
		LogPrintf(
				"TestBlockValidity::----CheckBlock---FALSE--------0000000000000000000000000000000000000000000000000\n");
		return false;
	}
	LogPrintf(
			"TestBlockValidity::----CheckBlock--------passed0000000000000000000000000000000000000000000000000\n");

	if (!ContextualCheckBlock(block, state, pindexPrev)) {
		LogPrintf(
				"TestBlockValidity::----ContextualCheckBlock---FALSE--------0000000000000000000000000000000000000000000000000\n");
		return false;
	}
	LogPrintf(
			"TestBlockValidity::----ContextualCheckBlock--------passed0000000000000000000000000000000000000000000000000\n");

	if (!ConnectBlock(block, state, &indexDummy, viewNew, true)) {
		LogPrintf(
				"TestBlockValidity::----ConnectBlock---FALSE--------0000000000000000000000000000000000000000000000000\n");
		return false;
	}
	LogPrintf(
			"TestBlockValidity::----ConnectBlock--------passed0000000000000000000000000000000000000000000000000\n");

	assert(state.IsValid());
	LogPrintf(
			"TestBlockValidity::----SUCCESSFUL--------0000000000000000000000000000000000000000000000000\n");
	return true;
}
...
bool CVerifyDB::VerifyDB(CCoinsView *coinsview, int nCheckLevel,
		int nCheckDepth) {
	LOCK(cs_main);
		...
		while (!blkdat.eof()) {
			...
			try {
				...
				if (mapBlockIndex.count(hash) == 0
						|| (mapBlockIndex[hash]->nStatus & BLOCK_HAVE_DATA)
								== 0) {
					CValidationState state;
					...
					if (state.IsError())
						break;
				} else if (hash != Params().HashGenesisBlock()
				...
				while (!queue.empty()) {
					...
					while (range.first != range.second) {
						...
						if (ReadBlockFromDisk(block, it->second)) {
							...
							if (ProcessNewBlock(dummy, NULL, &block,
									&it->second)) {
								nLoaded++;
				...
				LogPrintf("%s : Deserialize or I/O error - %s", __func__,
						e.what());
			}
		}
	} catch (std::runtime_error &e) {
...
void static CheckBlockIndex() {
	...
	std::pair<std::multimap<CBlockIndex*, CBlockIndex*>::iterator,
			std::multimap<CBlockIndex*, CBlockIndex*>::iterator> rangeGenesis =
			forward.equal_range(NULL);
	...
	while (pindex != NULL) {
		...
		assert(
				pindex->pprev == NULL || pindex->nChainWork >= pindex->pprev->nChainWork); // For every block except the genesis block, the chainwork must be larger than the parent's.
		assert(
				nHeight < 2
						|| (pindex->pskip && (pindex->pskip->nHeight < nHeight))); // The pskip pointer must point back for all but the first 2 blocks.
		...
		if (!CBlockIndexWorkComparator()(pindex,
				chainActive.Tip()) && pindexFirstMissing == NULL) {
		...
		std::pair<std::multimap<CBlockIndex*, CBlockIndex*>::iterator,
				std::multimap<CBlockIndex*, CBlockIndex*>::iterator> rangeUnlinked =
		...
		if (pindex->pprev
				&& pindex->nStatus
						& BLOCK_HAVE_DATA&& pindexFirstMissing != NULL) {
		...
		while (pindex) {
			...
			std::pair<std::multimap<CBlockIndex*, CBlockIndex*>::iterator,
					std::multimap<CBlockIndex*, CBlockIndex*>::iterator> rangePar =
					forward.equal_range(pindexPar);
			...
			if (inv.type == MSG_BLOCK || inv.type == MSG_FILTERED_BLOCK) {
				...
				if (!pushed && inv.type == MSG_MASTERNODE_WINNER) {
					if (masternodePayments.mapMasternodePayeeVotes.count(
						...
						ss.reserve(1000);
						ss
								<< masternodePayments.mapMasternodePayeeVotes[inv.hash];
				...
				if (!pushed && inv.type == MSG_BUDGET_PROPOSAL) {
					if (budget.mapSeenMasternodeBudgetProposals.count(
							inv.hash)) {
						CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
						ss.reserve(1000);
				...
				if (!pushed && inv.type == MSG_DSTX) {
					if (mapObfuscationBroadcastTxes.count(inv.hash)) {
						CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
						ss.reserve(1000);
						ss << mapObfuscationBroadcastTxes[inv.hash].tx
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv,
	...
	if (strCommand == "version") {
		...
		if (pfrom->nVersion != 0) {
			pfrom->PushMessage("reject", strCommand, REJECT_DUPLICATE,
		...
		LogPrintf(
				...
				pfrom->cleanSubVer, pfrom->nVersion, pfrom->nStartingHeight,
			...
			if (inv.type == MSG_BLOCK) {
				...
				if (!fAlreadyHave && !fImporting && !fReindex
					...
					if (chainActive.Tip()->GetBlockTime()
							> GetAdjustedTime() - Params().TargetSpacing() * 20
		...
		LogPrint("net", "getblocks %d to %s limit %d from peer=%d\n",
				...
				hashStop == uint256(0) ? "end" : hashStop.ToString(), nLimit,
				pfrom->id);
		...
		if (strCommand == "tx") {
			...
			if (pmn != NULL) {
				...
				if (!obfuScationSigner.VerifyMessage(pmn->pubKeyMasternode,
						vchSig, strMessage, errorMessage)) {
					LogPrintf(
				...
				if (!mapObfuscationBroadcastTxes.count(tx.GetHash())) {
					...
					mapObfuscationBroadcastTxes.insert(
							make_pair(tx.GetHash(), dstx));
				}
			}
		}
		...
		if (AcceptToMemoryPool(mempool, state, tx, true, &fMissingInputs)) {
			...
			for (unsigned int i = 0; i < vWorkQueue.size(); i++) {
				map<uint256, set<uint256> >::iterator itByPrev =
				...
				for (set<uint256>::iterator mi = itByPrev->second.begin();
					...
					const CTransaction& orphanTx =
							mapOrphanTransactions[orphanHash].tx;
					...
					if (setMisbehaving.count(fromPeer))
						continue;
					if (AcceptToMemoryPool(mempool, stateDummy, orphanTx, true,
							&fMissingInputs2)) {
		...
		if (state.IsInvalid(nDoS)) {
			LogPrint("mempool",
					...
					tx.GetHash().ToString(), pfrom->id, pfrom->cleanSubVer,
					state.GetRejectReason());
			pfrom->PushMessage("reject", strCommand, state.GetRejectCode(),
					state.GetRejectReason().substr(0,
							MAX_REJECT_MESSAGE_LENGTH), inv.hash);
			if (nDoS > 0)
	...
	// getaddr message mitigates the attack.
		...
		if (!(sProblem.empty())) {
			LogPrint("net",
					...
					pfrom->id, pfrom->cleanSubVer, sProblem,
			...
			try {
				...
				vRecv
						>> LIMITED_STRING(strMsg,
								CMessageHeader::COMMAND_SIZE) >> ccode >> LIMITED_STRING(strReason, MAX_REJECT_MESSAGE_LENGTH);
		...
		masternodePayments.ProcessMessageMasternodePayments(pfrom, strCommand,
				vRecv);
	...
	while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
		...
		if (nChecksum != hdr.nChecksum) {
			LogPrintf(
					...
					SanitizeString(strCommand), nMessageSize, nChecksum,
					hdr.nChecksum);
			continue;
		}
...
bool SendMessages(CNode* pto, bool fSendTrickle) {
	{
		...
		if (pto->nPingNonceSent == 0
				&& pto->nPingUsecStart + PING_INTERVAL * 1000000
						< GetTimeMicros()) {
		...
		BOOST_FOREACH(const CBlockReject& reject, state.rejects)
			pto->PushMessage("reject", (string) "block", reject.chRejectCode,
		...
		bool fFetch = state.fPreferredDownload
				|| (nPreferredDownload == 0 && !pto->fClient && !pto->fOneShot); // Download if this is a nice peer, or we have no nice peers and this one might do.
		if (!state.fSyncStarted && !pto->fClient && fFetch && !fImporting
				&& !fReindex) {
			...
			if (nSyncStarted == 0
				...
				CBlockIndex *pindexStart =
						pindexBestHeader->pprev ?
								pindexBestHeader->pprev : pindexBestHeader;
				LogPrint("net",
						"initial getheaders (%d) to peer=%d (startheight:%d)\n",
						pindexStart->nHeight, pto->id, pto->nStartingHeight);
				pto->PushMessage("getheaders",
						chainActive.GetLocator(pindexStart), uint256(0));
			}
		}
		...
		if (!pto->fDisconnect && !pto->fClient && fFetch
