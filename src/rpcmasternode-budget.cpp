...
Value mnbudget(const Array& params, bool fHelp)
{
    ...
    if (strCommand == "prepare") {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid CSB address");
    ...
    if (strCommand == "submit") {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'mnbudget submit proposal-name url payment_count block_start csbank_address monthly_payment_qrk fee_tx'");
